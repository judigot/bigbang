...
//====================GLOBAL VARIABLES====================//
var search_initial = "<input class='search-query' placeholder='Search'><div class='result-list'></div>";
var search_result = "<div class='search-result'></div>";
var search_selected = "<div class='selected-result-box'><span class='selected-result'></span></div>";
...
function search_searchable(elementId) {
    ...
    element.find("input").attr("list", elementId + "Datalist").css({
        "max-width": "350px"
    });
...
function search_setResult(elementId, rowValues) {
    ...
    element.find(".selected-result").attr({
    ...
    if (elementClass === "searchable-product") {
        $("#" + element.attr("id") + "Quantity").attr("max", rowValues["row_stock"]);
    }
    //=====CUSTOM=====//
}
...
function search_getResults(elementId, query) {
    ...
    $.ajax({
        ...
        if (!data.length) {
            element.find(".search-result").css({
                "color": "white",
                "width": "max-content",
                "margin": "auto",
            ...
            for (var i = 0; i < data.length; i++) {
                searchResults += "<div class='list-item' value='" + data[i]["row_identifier"] + "' data-row-id='" + data[i]["row_id"] + "' data-row-values='" + JSON.stringify(data[i]) + "'>" + data[i]["row_identifier"] + "</div>";
            }
            search_toggleResult(element);
