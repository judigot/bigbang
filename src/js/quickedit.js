...
//====================GENERAL====================//
$(function () {
    $(document).on("dblclick", "#customersTable td:not(:first-child), #productsTable td:not(:first-child)", function (e) {
        tableType = $(this).find("span").attr("data-table-type");
        oldValue = $(this).find("span").attr("value");
        referenceValue = $(this).children("span").attr("data-row-id") ? $(this).children("span").attr("data-row-id") : $(this).siblings("td:has(span)").children("span").attr("data-row-id");
    ...
    $(document).on("focusout keydown", $("#" + editor), function (e) {
        $("#" + editor).is(":visible") ? newValue = $("#" + editor).val().replace(/(\r\n|\n|\r)/gm, "") : false;
        if (e.keyCode === 13 || ($("#" + editor).is(":visible") && !$("#" + editor).is(":focus"))) {
...
function update(targetColumn, newValue, referenceValue) {
    $.ajax({
        ...
        custom_notify((tableType === "product" ? "Product" : "Customer") + " was successfully updated.", null);
        tableType === "product" ? product_productsTable.ajax.reload() : customer_customersTable.ajax.reload();
    }).fail(function (data) {});
}
...
function deleteTableRow(referenceValue) {
    $.ajax({
        ...
        custom_notify(custom_capitalizeFirstLetter(tableType) + " was successfully deleted.", null);
        switch (tableType) {
            case "order":
